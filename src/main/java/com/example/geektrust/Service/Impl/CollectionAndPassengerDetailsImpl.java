...
public class CollectionAndPassengerDetailsImpl implements CollectionAndPassengerDetails {
    ...
    public List<String> calculateCollection(List <MetroCard> metroCardList, List <Journey> journeyList) {
        ...
        for(int i=0;i< journeyList.size();i++) {
            String curPassenger = journeyList.get(i).getPassenger();
            Destination curDestination=journeyList.get(i).getDestination();
            int fairAmount=0;
            List <Integer> ansList=null;
            MetroCard curMetroCard=null;
            int curFair=0,curDiscount=0;
            ...
            for (int j = 0; j < metroCardList.size(); j++) {
                if (journeyList.get(i).getMetroCardId().equals(metroCardList.get(j).getMetroCardID())) {
                    curMetroCard = metroCardList.get(j);
                }
            }
    ...
    public ArrayList<Integer> init(String passengerType, int fairAmount, MetroCard curMetroCard, PassengerCount passengerCount) {
        Integer ticketAmount=0,discountAmount=0;
    ...
    public List<String> getPassenegrCount(List <String> ansList,PassengerCount passengerCount) {
        if(passengerCount.getKidsCount()>0) {
            String curLine="KID "+String.valueOf(passengerCount.getKidsCount());
            ansList.add(curLine);
        }
        if(passengerCount.getAdultCount()>0) {
            String curLine="ADULT "+String.valueOf(passengerCount.getAdultCount());
            ansList.add(curLine);
        }
        if(passengerCount.getSeniorCitizenCount()>0) {
            String curLine="SENIOR_CITIZEN "+String.valueOf(passengerCount.getSeniorCitizenCount());
            ansList.add(curLine);
        }
        return ansList;
    }
